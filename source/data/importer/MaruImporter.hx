...
import meta.state.editors.charting.handlers.ChartEditorNotificationHandler;
...
class MaruImporter
{
    public static function parseMaruDataRaw(input:String, fileName:String = 'raw'):MaruData
    {
        ...
        {
            trace('[FNFMaruImporter] Error parsing JSON data from ' + fileName + ':');
    ...
    public static function migrateMetadata(songData:MaruData, difficulty:String = 'normal'):SongMetadata
    {
        ...
        songMetadata.generatedBy = 'Chart Editor Import (FNF Maru)';
        ...
        songMetadata.playData.characters = new SongCharacterData(songData?.song?.players[0] ?? 'bf', songData?.song?.players[2] ?? 'gf', songData?.song?.players[1] ?? 'dad');
