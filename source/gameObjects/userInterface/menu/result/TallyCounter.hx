    ...
    function drawNumbers()
    {
        ...
        for (ind => num in seperatedScore)
        {
            if (ind >= members.length)
            {
                ...
                if (this.align == FlxTextAlign.RIGHT) xPos -= (fullNumberDigits * (43 * this.scale.x));
...
class TallyNumber extends FunkinSprite
{
    public function new(x:Float, y:Float, digit:Int)
    {
        super(x, y);

        loadFrame("menus/base/resultScreen/tallieNumber");
