...
class Pico extends Backcard
{
    ...
    override function applyExitMovers(?exitMovers:ExitMoverData, ?exitMoversCharSel:ExitMoverData)
    {
    ...
    override function init()
    {
        ...
        scrollBack = new FlxBackdrop(Paths.image('$folder/backingCards/pico/lowerLoop'), X, 20);
        ...
        scrollLower = new FlxBackdrop(Paths.image('$folder/backingCards/pico/lowerLoop'), X, 20);
        scrollLower.setPosition(0, 406);
        scrollLower.velocity.x = -110;
        add(scrollLower);
    
        blueBar = new FlxSprite(0, 239).loadGraphic(Paths.image('$folder/backingCards/pico/blueBar'));
        ...
        scrollTop.frames = Paths.getSparrowAtlas('$folder/backingCards/pico/topLoop');
        ...
        scrollMiddle = new FlxBackdrop(Paths.image('$folder/backingCards/pico/middleLoop'), X, 15);
        scrollMiddle.setPosition(0, 346);
        add(scrollMiddle);
        scrollMiddle.velocity.x = 220;
    
        glowDark = new FlxSprite(-300, 330).loadGraphic(Paths.image('$folder/backingCards/pico/glow'));
        glowDark.blend = BlendMode.MULTIPLY;
        add(glowDark);
    
        glow = new FlxSprite(-300, 330).loadGraphic(Paths.image('$folder/backingCards/pico/glow'));
        ...
        confirmAtlas = new FlxAtlasSprite(5, 55, Paths.animateAtlas('$folder/backingCards/pico/pico-confirm'));
    ...
    override function confirm()
    {
        ...
        FlxTween.color(FreeplayState.instance.bgDad, 10 / 24, 0xFFFFFFFF, 0xFF8A8A8A,
          {
            ease: FlxEase.expoOut,
            onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
            }
          });
    
        new FlxTimer().start(10 / 24, function(_) {
          // shoot
          FlxTween.color(FreeplayState.instance.bgDad, 3 / 24, 0xFF343036, 0xFF696366,
            {
              ease: FlxEase.expoOut,
              onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
        ...
        new FlxTimer().start(14 / 24, function(_) {
          // shoot
          FlxTween.color(FreeplayState.instance.bgDad, 3 / 24, 0xFF27292D, 0xFF686A6F,
            {
              ease: FlxEase.expoOut,
              onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
        ...
        new FlxTimer().start(18 / 24, function(_) {
          // shoot
          FlxTween.color(FreeplayState.instance.bgDad, 3 / 24, 0xFF2D282D, 0xFF676164,
            {
              ease: FlxEase.expoOut,
              onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
        ...
        new FlxTimer().start(21 / 24, function(_) {
          // shoot
          FlxTween.color(FreeplayState.instance.bgDad, 3 / 24, 0xFF29292F, 0xFF62626B,
            {
              ease: FlxEase.expoOut,
              onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
        ...
        new FlxTimer().start(24 / 24, function(_) {
          // shoot
          FlxTween.color(FreeplayState.instance.bgDad, 3 / 24, 0xFF29232C, 0xFF808080,
            {
              ease: FlxEase.expoOut,
              onUpdate: function(_) {
                FreeplayState.instance.angleMaskShader.extraColor = FreeplayState.instance.bgDad.color;
