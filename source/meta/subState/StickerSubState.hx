...
class StickerSubState extends MusicBeatSubState
{
    ...
    function regenStickers():Void
    {
        ...
        var xPos:Float = -100;
        var yPos:Float = -100;
        var stickerList:Array<String> = CoolUtil.coolTextFile(Paths.txt('images/transitionSwag/stickersList'));

        while (xPos <= FlxG.width)
        {
            var stickerPack:String = FlxG.random.getObject(stickerList);

            var stickerInfo:StickerInfo = new StickerInfo(stickerPack);
...
class StickerInfo
{
    ...
    {
        var path = Paths.json('images/transitionSwag/' + stickerSet + '/stickers');
        var json = haxe.Json.parse(openfl.utils.Assets.getText(path));     
