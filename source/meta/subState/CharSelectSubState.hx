...
import gameObjects.userInterface.menu.charSelect.*;
...
class CharSelectSubState extends MusicBeatSubState
{
  ...
  var playerChill:Player;
  var playerChillOut:Player;
  var gfChill:GF;
  ...
  var folder:String = 'menus/base/charSelect';
  override public function create():Void
  {
    super.create();

    bopInfo = FramesJSFLParser.parse(Paths.file('images/$folder/iconBopInfo/iconBopInfo.txt'));

    var bg:FlxSprite = new FlxSprite(-153, -140);
    bg.loadGraphic(Paths.image('$folder/charSelectBG'));
    bg.scrollFactor.set(0.1, 0.1);
    add(bg);

    var crowd:FlxAtlasSprite = new FlxAtlasSprite(0, 0, Paths.animateAtlas('$folder/crowd'));
    crowd.anim.play();
    ...
    stageSpr.frames = Paths.getSparrowAtlas('$folder/charSelectStage');
    ...
    curtains.loadGraphic(Paths.image('$folder/curtains'));
    curtains.scrollFactor.set(1.4, 1.4);
    add(curtains);

    barthing = new FlxAtlasSprite(0, 0, Paths.animateAtlas('$folder/barThing'));
    barthing.anim.play("");
    ...
    charLight.loadGraphic(Paths.image('$folder/charLight'));
    add(charLight);

    var charLightGF:FlxSprite = new FlxSprite(180, 240);
    charLightGF.loadGraphic(Paths.image('$folder/charLight'));
    add(charLightGF);
    ...
    playerChillOut = new Player(0, 0);
    playerChillOut.switchChar("bf");
    playerChillOut.visible = false;
    add(playerChillOut);

    playerChill = new Player(0, 0);
    playerChill.switchChar("bf");
    add(playerChill);

    var speakers:FlxAtlasSprite = new FlxAtlasSprite(0, 0, Paths.animateAtlas('$folder/charSelectSpeakers'));
    speakers.anim.play("");
    ...
    fgBlur.loadGraphic(Paths.image('$folder/foregroundBlur'));
    fgBlur.blend = openfl.display.BlendMode.MULTIPLY;
    add(fgBlur);

    dipshitBlur = new FlxSprite(419, -65);
    dipshitBlur.frames = Paths.getSparrowAtlas('$folder/dipshitBlur');
    ...
    dipshitBacking.frames = Paths.getSparrowAtlas('$folder/dipshitBacking');
    ...
    chooseDipshit.loadGraphic(Paths.image('$folder/chooseDipshit'));
    add(chooseDipshit);
    ...
    cursor.loadGraphic(Paths.image('$folder/charSelector'));
    cursor.color = 0xFFFFFF00;

    // FFCC00

    cursorBlue = new FlxSprite(0, 0);
    cursorBlue.loadGraphic(Paths.image('$folder/charSelector'));
    cursorBlue.color = 0xFF3EBBFF;

    cursorDarkBlue = new FlxSprite(0, 0);
    cursorDarkBlue.loadGraphic(Paths.image('$folder/charSelector'));
    ...
    cursorConfirmed.frames = Paths.getSparrowAtlas('$folder/charSelectorConfirm');
    ...
    cursorDenied.frames = Paths.getSparrowAtlas('$folder/charSelectorDenied');
    ...
    FlxG.camera.addShader(fadeShader);
    
    Conductor.stepHit.add(spamOnStep);
    // FlxG.debugger.track(temp, "tempBG");

    transitionGradient = new FlxSprite(0, 0).loadGraphic(Paths.image('$folder/transitionGradient'));
  ...
  function initLocks():Void
  {
    ...
    for (i in 0...9)
    {
      if (availableChars.exists(i) && Init.trueSettings.get("Playable Character").contains(availableChars[i]))
      {
        var path:String = availableChars.get(i);
        var temp:PixelatedIcon = new PixelatedIcon(path);
  ...
  function unLock()
  {
    ...
    selectTimer.start(0.5, function(_) {
      ...
      lock.onAnimationComplete.addOnce(function(_) {
        ...
        var icon = new PixelatedIcon(char);
        ...
        Init.trueSettings.get("Playable Character").push(char);
  ...
  override public function update(elapsed:Float):Void
  {
    ...
    if (availableChars.exists(getCurrentSelected()) && Init.trueSettings.get("Playable Character").contains(availableChars[getCurrentSelected()]))
    {
  ...
  function doBop(icon:PixelatedIcon, elapsed:Float):Void
  {
    ...
    if (bopTimer >= delay)
    {
      ...
      var scaleXDiff:Float = curFrame.scaleX - refFrame.scaleX - 10;
      var scaleYDiff:Float = curFrame.scaleY - refFrame.scaleY - 10;
