...
import gameObjects.userInterface.menu.freeplay.background.base.*;
import gameObjects.userInterface.menu.freeplay.background.*;
import gameObjects.userInterface.menu.freeplay.*;
import openfl.display.BlendMode;
import effect.IntervalShake;
...
class FreeplayState extends MusicBeatSubState
{
  ...
  var backingCard:Null<Backcard> = null;
  ...
  public function new(?params:FreeplayStateParams, ?stickers:StickerSubState)
  {
    ...
    {
      case 'bf':
        backingCard = new Boyfriend();
      case 'pico':
        backingCard = new Pico();
      default:
        backingCard = new Backcard(currentCharacterId);
    }
    ...
    rankCamera = funnyCam = new FunkinCamera();
    grpCapsules = new FlxTypedGroup<SongMenuItem>();
    grpDifficulties = new FlxTypedSpriteGroup<DifficultySprite>(-300, 80);
    letterSort = new LetterSort(400, 75);
    rankBg = new FunkinSprite();
    rankVignette = new FunkinSprite().loadImage('menus/base/freeplay/rankVignette');
    sparks = new FunkinSprite();
    sparksADD = new FunkinSprite();
    txtCompletion = new AtlasText(1185, 87, '69', 'menus/base/freeplay/freeplay-clear');

    ostName = new FlxText(8, 8, FlxG.width - 8 - 8, 'OFFICIAL OST', 48);
    charSelectHint = new FlxText(-40, 18, FlxG.width - 8 - 8, 'Press [ LOL ] to change characters', 32);
    
    bgDad = new FlxSprite(backingCard.pinkBack.width * 0.74, 0).loadGraphic(styleData.getBgAssetGraphic() ?? FlxG.bitmap.add(Paths.image('menus/base/freeplay/freeplayBGdad')));
  }
  ...
  override function create():Void
  {
    ...
    for (levelId in CoolUtil.coolTextFile(Paths.txt('levels/list')))
    {
    ...
    charSelectHint.text = 'Press [TAB] to change characters';
    ...
    fnfHighscoreSpr.frames = Paths.getSparrowAtlas('menus/base/freeplay/highscore');
    ...
    var clearBoxSprite:FlxSprite = new FlxSprite(1165, 65).loadGraphic(Paths.image('menus/base/freeplay/clearBox'));
    ...
    var diffSelLeft:DifficultySelector = new DifficultySelector(20, grpDifficulties.y - 10, false, styleData);
    var diffSelRight:DifficultySelector = new DifficultySelector(325, grpDifficulties.y - 10, true, styleData);
  ...
  function rankAnimStart(fromResults:FromResultsParams):Void
  {
    ...
    if (fromResults.oldRank != null)
    {
      ...
      sparks.loadFrame('menus/base/freeplay/sparks');
      ...
      sparksADD.loadFrame('menus/base/freeplay/sparksadd');
  ...
  {
    var transitionGradient = new FlxSprite(0, 720).loadGraphic(Paths.image('menus/base/freeplay/transitionGradient'));
  ...
  override function update(elapsed:Float):Void
  {
    ...
    if (FlxG.keys.justPressed.TAB && !busy)
    {
      tryOpenCharSelect();
    }
  ...
  function changeDiff(change:Int = 0, force:Bool = false):Void
  {
    ...
    currentDifficultyIndex = FlxMath.wrap(currentDifficultyIndex + change, 0, difficultiesAvailable.length-1);
    ...
    if (daSong != null)
    {
      ...
      var songScore:Null<SaveScoreData> = Highscore.instance.getSongScore(daSong.data.id, currentDifficulty, currentVariation);
  ...
  function openInstrumentalList(cap:SongMenuItem, instrumentalIds:Array<String>):Void
  {
    ...
    capsuleOptionsMenu = new CapsuleOptionsMenu(cap.x + 175, cap.y + 115, instrumentalIds);
  ...
  function changeSelection(change:Int = 0):Void
  {
    ...
    {
      var songScore:Null<SaveScoreData> = Highscore.instance.getSongScore(daSongCapsule.freeplayData.data.id, currentDifficulty, currentVariation);
      intendedScore = songScore?.score ?? 0;
      intendedCompletion = songScore == null ? 0.0 : ((songScore.tallies.sick + songScore.tallies.good) / songScore.tallies.totalNotes);
  ...
  public function playCurSongPreview(?daSongCapsule:SongMenuItem):Void
  {
    ...
    else
    {
      ...
      trace('start: ${songDifficulty.previewStart} end: ${songDifficulty.previewEnd}');
...
class DifficultySelector extends FlxSprite
{
  ...
  public function new(x:Float, y:Float, flipped:Bool, ?styleData:FreeplayStyle = null)
  {
    super(x, y);

    frames = Paths.getSparrowAtlas(styleData.getSelectorAssetKey() ?? 'menus/base/freeplay/selector/default');
  ...
  override function update(elapsed:Float):Void
  {
    if (flipX && FreeplayState.instance.getControls().UI_RIGHT_P && !FreeplayState.instance.busy) moveShitDown();
    if (!flipX && FreeplayState.instance.getControls().UI_LEFT_P && !FreeplayState.instance.busy) moveShitDown();
...
class FreeplaySongData
{
  ...
  public function new(data:Song, levelData:Level)
  {
    ...
    this.isFav = Highscore.instance.isSongFavorited(data.songName);
  }
  ...
  public function toggleFavorite():Bool
  {
    ...
    if (isFav)
    {
		Highscore.instance.favoriteSong(data.songName);
    }
    else
    {
		Highscore.instance.unfavoriteSong(data.songName);
    }
    return isFav;
  }
  ...
  function get_scoringRank():Null<ScoringRank>
  {
    ...
    return Highscore.instance.getSongRank(data.id, FreeplayState.rememberedDifficulty, variation);
  }
}
...
class DifficultySprite extends FlxSprite
{
  ...
  public function new(diffId:String)
  {
    ...
    while (!Assets.exists(Paths.image('menus/base/freeplay/difficulties/${assetDiffId}')))
    {
    ...
    if (Assets.exists(Paths.file('images/menus/base/freeplay/difficulties/${assetDiffId}.xml')))
    {
    ...
    {
      this.loadGraphic(Paths.image('menus/base/freeplay/difficulties/${assetDiffId}'));
