...
class NoteSplashDebugState extends MusicBeatState
{
	...
	var notes:FlxTypedGroup<UIStaticArrow>;
	...
	override function create()
	{
		...
		notes = new FlxTypedGroup<UIStaticArrow>();
		add(notes);
		...
		for (i in 0...maxNotes)
		{
			var x = i * 220 + 240;
			var y = 290;
			var note:UIStaticArrow = new UIStaticArrow(x, y, i);
			note.alpha = 0.75;
		...
		imageInputText = new FlxInputText(txtx, txty - 30, 360, 'noteSplashes', 16);
		blockPressWhileTypingOn.push(imageInputText);
		add(imageInputText);

		var reloadImage:FlxButton = new FlxButton(imageInputText.x - 50, imageInputText.y - 3, "Reload Image", function()
		{
			loadCustomSplash(imageInputText.text);
		});
		add(reloadImage);

		var animName:FlxText = new FlxText(txtx, imageInputText.y  - 15, 'Image:', 16);
		add(animName);
		...
		var assetModifiers:Array<String> = CoolUtil.returnAssetsLibrary('noteskins/notes/default');
		var assetModifierDropDown:FlxUIDropDownMenu = new FlxUIDropDownMenu(txtx, txty - 90, FlxUIDropDownMenu.makeStrIdLabelArray(assetModifiers, true), function(character:String)
		{
			assetModifier = assetModifiers[Std.parseInt(character)];
			notes.forEachAlive(function(spr:UIStaticArrow) {
				spr.reloadNote(assetModifier);
				spr.playAnim('static');
			});
		});
		assetModifierDropDown.selectedLabel = assetModifier;
		add(assetModifierDropDown);
		
		var animName:FlxText = new FlxText(txtx, assetModifierDropDown.y  - 15, 'Asset Modifier:', 16);
		add(animName);
	...
	function loadFrames()
	{
		texturePath = 'noteskins/notes/default/$assetModifier/noteSplashes';
		splashes.forEachAlive(function(spr:FlxSprite) {
			spr.frames = Paths.getSparrowAtlas('noteskins/notes/default/$assetModifier/noteSplashes');
		...
		config = NoteSplash.precacheConfig('noteSplashes', assetModifier);
	...
	function loadCustomSplash(image)
	{
		
		texturePath = 'noteskins/notes/default/$assetModifier/$image';
		splashes.forEachAlive(function(spr:FlxSprite) {
		...
		// Initialize config
		NoteSplash.configs.clear();
		config = NoteSplash.precacheConfig(image, assetModifier);
		nameInputText.text = config.anim;
		stepperMinFps.value = config.minFps;
		stepperMaxFps.value = config.maxFps;
	...
	function saveFile()
	{
		var maxLen:Int = maxAnims * NoteSplash.colArray.length;
		...
		if (strToSave.length > 0)
		{
			...
			_file.save(strToSave, '${imageInputText.text}.txt');
		}
	}
	...
	function reloadAnims()
	{
		...
		while(loopContinue)
		{
			...
			splashes.forEachAlive(function(spr:FlxSprite)
			{
				for (i in 0...NoteSplash.colArray.length) {
					var animName = 'note$i-$animID';
					if (!addAnimAndCheck(spr, animName, '${config.anim} ${NoteSplash.colArray[i]} $animID', 24, false)) {
						loopContinue = false;
						return;
					}
	...
	{
		var max:Int = NoteSplash.colArray.length;
		curSelected += change;
	...
	function selectedArray(sel:Int = -1)
	{
		...
		var animID:Int = sel + ((curAnim - 1) * NoteSplash.colArray.length);
