...
class ChartEditorEventSprite extends FlxSprite
{
  ...
  static function buildFrames(force:Bool = false):FlxFramesCollection
  {
    ...
    var defaultFrames:FlxAtlasFrames = Paths.getSparrowAtlas('editor/chart/events/$DEFAULT_EVENT');
    ...
    {
      var exists:Bool = Assets.exists(Paths.image('editor/chart/events/$eventName'));
      if (!exists) continue; // No graphic for this event.

      var frames:FlxAtlasFrames = Paths.getSparrowAtlas('editor/chart/events/$eventName');
