package meta.state.editors.charting;
...
import meta.ui.components.CharacterPlayer;
import audio.waveform.WaveformSprite;
import audio.visualize.PolygonSpectogram;
import meta.state.editors.charting.commands.*;
import meta.state.editors.charting.components.*;
import meta.state.editors.charting.handlers.*;
import meta.state.editors.charting.toolboxes.*;
...
@:build(haxe.ui.ComponentBuilder.build("assets/exclude/ui/chart-editor/main-view.xml"))
class ChartEditorState extends UIState // UIState derives from MusicBeatState
{
  ...
  {
    if(chartEditorSave.data.chartEditorHasBackup != null) return chartEditorSave.data.chartEditorHasBackup;
    return false;
  }

  function set_shouldShowBackupAvailableDialog(value:Bool):Bool
  {
    if(chartEditorSave.data.chartEditorHasBackup != null) chartEditorSave.data.chartEditorHasBackup = value;
    return value;
  }
  ...
  function get_currentSongMetadata():SongMetadata
  {
    ...
    if (result == null)
    {
      result = new SongMetadata('Default Song Name', selectedVariation);
  ...
  /**
   * The `Edit -> Change Kind Notes` menu item.
  */
  var menubarItemKindNotes:MenuItem;
  ...
  public function loadPreferences():Void
  {
    ...
    {
      if(chartEditorSave.data.chartEditorPreviousFiles != null) previousWorkingFilePaths = [null].concat(chartEditorSave.data.chartEditorPreviousFiles);
    }
    else
    {
      if(chartEditorSave.data.chartEditorPreviousFiles != null) previousWorkingFilePaths = [currentWorkingFilePath].concat(chartEditorSave.data.chartEditorPreviousFiles);
    }
    if(chartEditorSave.data.noteSnapQuantIndex != null) noteSnapQuantIndex = chartEditorSave.data.chartEditorNoteQuant;
    if(chartEditorSave.data.chartEditorLiveInputStyle != null) currentLiveInputStyle = chartEditorSave.data.chartEditorLiveInputStyle;
    if(chartEditorSave.data.chartEditorDownscroll != null) isViewDownscroll = chartEditorSave.data.chartEditorDownscroll;
    if(chartEditorSave.data.chartEditorPlaytestStartTime != null) playtestStartTime = chartEditorSave.data.chartEditorPlaytestStartTime;
    if(chartEditorSave.data.chartEditorTheme != null)  currentTheme = chartEditorSave.data.chartEditorTheme;
    if(chartEditorSave.data.chartEditorMetronomeVolume != null) metronomeVolume = chartEditorSave.data.chartEditorMetronomeVolume;
    if(chartEditorSave.data.chartEditorHitsoundVolumePlayer != null)  hitsoundVolumePlayer = chartEditorSave.data.chartEditorHitsoundVolumePlayer;
    if(chartEditorSave.data.chartEditorHitsoundVolumeOpponent != null) hitsoundVolumePlayer = chartEditorSave.data.chartEditorHitsoundVolumeOpponent;
    if(chartEditorSave.data.chartEditorThemeMusic != null) this.welcomeMusic.active = chartEditorSave.data.chartEditorThemeMusic;
  ...
  {
    menuBG = new FlxSprite().loadGraphic(Paths.image('menus/base/menuDesat'));
    add(menuBG);
  ...
  function buildAdditionalUI():Void
  {
    ...
    buttonSelectEvent.icon = Paths.image('editor/chart/events/Default');
  ...
  function setupUIListeners():Void
  {
    ...
    menubarItemKindNotes.onClick = _ -> performCommand(new SetKindNotesCommand(currentNoteSelection, noteKindToPlace));
  ...
  function setupAutoSave():Void
  {
    ...
    lime.app.Application.current.onExit.add(onWindowClose);
    saveDataDirty = false;
  }
  ...
  {
    lime.app.Application.current.onExit.remove(onWindowClose);
  }
  ...
  function handlePlayerPreviewToolbox():Void
  {
    ...
    if (playerPreviewDirty)
    {
      ...
      if (currentSongMetadata.playData.characters.player != charPlayer.charId)
      {
        ...
        {
          healthIconBF.char = CharacterRegistry.fetchCharacterData(currentSongMetadata.playData.characters.player).healthIcon.id;
        }
    ...
    if (opponentPreviewDirty)
    {
      ...
      if (currentSongMetadata.playData.characters.opponent != charPlayer.charId)
      {
        if (healthIconDad != null)
        {
          ...
          healthIconDad.char = CharacterRegistry.fetchCharacterData(currentSongMetadata.playData.characters.opponent).healthIcon.id;
        }
  ...
  function handleHealthIcons():Void
  {
    if (healthIconsDirty)
    {
      ...
      if (healthIconBF != null)
      {
        ...
        healthIconBF.scale.x *= healthIconBF.scale.y *= 0.5; // Make the icon smaller in Chart Editor.
      ...
      if (healthIconDad != null)
      {
        ...
        healthIconDad.scale.x *= healthIconDad.scale.y *= 0.5;// Make the icon smaller in Chart Editor.
      }
  ...
  function handleEditKeybinds():Void
  {
    ...
    // ALT = Change Note Kind
    if (FlxG.keys.pressed.ALT)
    {
      // Change Kind selected notes.
      performCommand(new SetKindNotesCommand(currentNoteSelection, noteKindToPlace));
    }
